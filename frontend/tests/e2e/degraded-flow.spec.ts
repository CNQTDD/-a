...
test("degraded retrieval routes the workbench to manual handling", async ({
  page,
}) => {
  ...
  await page.goto("/");
  await page.getByLabel("Complaint text").fill("请帮我核查套餐扣费");
  await page.getByRole("button", { name: "Start workflow" }).click();

  await expect(page.getByTestId("fallback-notice")).toHaveText(
    "Elasticsearch-only fallback active",
  );
  await expect(page.getByText("Manual handling required", { exact: true })).toBeVisible();
  await expect(page.getByTestId("workflow-timeline")).toContainText(
    "Workflow error: Milvus unavailable, Elasticsearch-only fallback active",
  );
