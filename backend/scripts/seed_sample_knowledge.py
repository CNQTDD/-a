...
from app.db.models.complaint import KnowledgeDocument
...
def _vector(seed_text: str) -> list[float]:
    ...
    return [round(int.from_bytes(digest[i : i + 4], "big") / 2**32, 6) for i in range(0, 16, 4)]
...
def _build_documents(rules_path: Path, complaints_path: Path) -> list[dict[str, object]]:
    ...
    for index, rule in enumerate(_parse_rules(rules_path), start=1):
        business_type = next(
            (
                value
                for key, value in rule_business_types.items()
                if key in rule["title"]
            ),
            "billing",
        )
        docs.append(
...
def _seed_sqlalchemy(database_url: str, documents: list[dict[str, object]]) -> None:
    ...
    with session_factory() as session:
        for document in documents:
            ...
            if existing is None:
                session.add(KnowledgeDocument(**{k: v for k, v in document.items() if k != "full_text"}))
            else:
...
def main() -> int:
    ...
    parser.add_argument("--rules-file", type=Path, default=Path("..") / "data" / "samples" / "rules.md")
    parser.add_argument("--complaints-file", type=Path, default=Path("..") / "data" / "samples" / "complaints.jsonl")
